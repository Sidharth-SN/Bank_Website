...
class Users(db.Model):
    ...
    banks = db.relationship('Accounts', backref = 'users', passive_deletes = True)
...
class Accounts(db.Model):
    ...
    c_card = db.relationship('Credit_card', backref = 'cacc', passive_deletes = True)
    d_card = db.relationship('Debit_card', backref = 'dacc', passive_deletes = True)
    t_acc = db.relationship('Transactions', backref = 'trans', passive_deletes = True)
    loan = db.relationship('Loans', backref = 'loans', passive_deletes = True)
...
class Transactions(db.Model):
    ...
    tAmount = db.Column(db.Integer, default = 0)
    acc_to = db.Column(db.Integer, nullable = False)
